...
| ACU billing | 1 ACU = $0.01. **100 ACU free** on every account; top-ups $5 / $10 / $15. The cost multiplier is **internal** — `publicCharge()` is the only shape that may cross an API boundary, and `/api/ai` no longer returns provider cost, markup or which vendor served the call. | `src/shared/constants/billing.ts` — 17 tests |
