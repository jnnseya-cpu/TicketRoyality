...
| Door scanner | Per-event scan page, QR read, redeem. **Redemption is a read-then-write with no transaction** — two doors scanning one ticket at the same moment can both admit. Fix built but not wired; see B2 below. | `/events/[id]/check-in` |
...
| **QR signing + atomic redemption (B2)** | **Groundwork committed, NOT wired — the live door scanner is unchanged.** Built and compiling: an HMAC helper in `functions/`, tickets signed at issuance (`qrSignature` on the ticket doc), a shared payload codec, and `redeemAtDoor()` which runs the redemption **in a Firestore transaction**, verifies the signature against a freshly recomputed HMAC, and checks server-side that the caller owns the event. Still to do: the API route and pointing `TicketScanner` at it. Until then redemption still runs from the organiser's browser via `redeemTicket()`. **Correction to an earlier claim in this file:** an unsigned QR is *not* forgeable into a free entry — redemption requires a real ticket document with `status: 'valid'` for that event. The genuine defects are (a) `redeemTicket()` is a read-then-write with **no transaction**, so two doors scanning one ticket simultaneously can both admit, and (b) the browser is the authority. A screenshot still works **once** by design; defeating that needs rotating codes, which are not built. | `backend/services/redeem.ts`, `functions/src/qr.ts` |
